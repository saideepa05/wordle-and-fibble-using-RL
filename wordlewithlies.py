...
# View Runs:
# Wordle with 1 lie: https://wandb.ai/worldunknown/world/runs/r4fwnt44?workspace=user-deepanaidu0501
# Wordle with 2 lie: https://wandb.ai/worldunknown/world/runs/x162b2ga?workspace=user-deepanaidu0501
# wordle with 3 lie: https://wandb.ai/worldunknown/world/runs/xylquk0r?workspace=user-deepanaidu0501
# Wordle with 4 lie: https://wandb.ai/worldunknown/world/runs/v0zwd0bm?workspace=user-deepanaidu0501
# Wordle with 5 lie: https://wandb.ai/worldunknown/world/runs/ypankmfa?workspace=user-deepanaidu0501
...
class Wordle:
    ...
    def _getpattern(self, guess: str):
        # Initialize pattern with None values for each letter in the guess
        pattern = [None] * self.n_letters

        # Temporarily store letters from the answer that have not been matched yet
        letters_left = list(self.answer)
        ...
        # Second pass: Find misplaced (yellow) letters
        for i, g in enumerate(guess):
            if pattern[i] is None:  # Skip already marked as correct
                if g in letters_left:
                    pattern[i] = 'Y'
                    letters_left.remove(g)
                else:
                    pattern[i] = 'B'  # Letter not in the answer at all

        # Check if the guess is correct
        iscorrect = all(p == 'G' for p in pattern)

        # Introduce lies in the pattern
        num_lies = 1  # For example, introducing 2 lies
        pattern = self._introduce_lie(pattern, num_lies)

        return pattern, iscorrect

    def _introduce_lie(self, pattern, num_lies=1):
        possible_positions = [i for i, p in enumerate(pattern) if p != 'G'] 
        random.shuffle(possible_positions)
        lies_introduced = 0

        for lie_position in possible_positions:
            if lies_introduced >= num_lies:
                break  # Exit if the desired number of lies have been introduced

            original = pattern[lie_position]
            new_values = self._get_new_values_for_lie(original)

            # Apply a new value if possible
            if new_values:
                pattern[lie_position] = random.choice(new_values)
                lies_introduced += 1

        return pattern

    def _get_new_values_for_lie(self, original):
        if original == 'G':
            return ['Y', 'B']  # It's unlikely to change 'G', but just in case
        elif original == 'Y':
            return ['G', 'B']
        elif original == 'B':
            return ['G', 'Y']
...
import wandb
wandb.login(key='Enter your Key') # Enter your API Key
...
# some configs
rewards = {'repeat':-0.1, 'B':0, 'Y':3, 'G':30, 'win':60, 'lose':-50, 'step':-5}
